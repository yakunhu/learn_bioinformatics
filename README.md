...
The `drill-syntax-bioinformatics` Skill draws initial quickfire questions from curated Python, R, and statistics banks organized by practical categories, then generates review variants that test the same objective with changed details.
...
To generate a new category, first decide whether you want quickfire or full questions, then ask Codex to extend the appropriate bank with `drill-syntax-bioinformatics` or the full-question curriculum with the programming-tutor workflow; there is currently no dedicated add-category Skill.
...
Name the track or category you want—for example, **“Pandas”**—and the Skill selects that category and presents one question at a time.
